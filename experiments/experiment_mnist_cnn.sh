    ...
    do
        ...
        do
            if [ $prior = "student-t" ]; then
                for df in "${dfs[@]}"
                do
                    bsub -n 2 -W 24:00 -J "bnn" -sp 40 -g /vincent/experiments -G ms_raets -R "rusage[mem=8000,ngpus_excl_p=1]" "source activate bnn; python train_bnn.py with weight_prior=$prior data=mnist inference=SGLD model=classificationconvnet width=64 warmup=30 burnin=10 skip=1 n_samples=100 lr=0.01 weight_scale=$scale cycles=20 batch_size=128 temperature=$temp save_samples=True log_dir=$logdir weight_prior_params={"'"'"df"'"'":$df}"
                done
...
bsub -n 2 -W 24:00 -J "bnn" -sp 40 -g /vincent/experiments -G ms_raets -R "rusage[mem=8000,ngpus_excl_p=1]" "source activate bnn; python train_bnn.py with weight_prior=improper data=mnist inference=SGLD model=classificationconvnet width=64 warmup=30 burnin=10 skip=1 n_samples=100 lr=0.01 weight_scale=1.41 cycles=20 batch_size=128 temperature=0.0 save_samples=True log_dir=$logdir"
