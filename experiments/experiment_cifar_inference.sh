...
bsub -n 2 -W 120:00 -J "bnn" -sp 40 -g /vincent/experiments -G ms_raets -R "rusage[mem=32000,ngpus_excl_p=1]" "source activate bnn; python train_bnn.py with weight_prior=improper data=cifar10 inference=SGLD model=$model warmup=30 burnin=15 skip=1 n_samples=100 lr=$lr momentum=0.98 weight_scale=1.41 cycles=20 batch_size=128 temperature=0.0 save_samples=True log_dir=$logdir batchnorm=True"
